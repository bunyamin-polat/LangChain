...
async def main():
    ...
    if isinstance(res, str) or "results" not in res:
        log_error(f"TavilyCrawl: Crawl failed - {res}")
        return
    ...
    for tavily_crawl_result_item in res["results"]:
        ...
        if not raw_content:
            log_warning(
                f"TavilyCrawl: Skipping {tavily_crawl_result_item['url']} - no content extracted"
            )
            continue
        log_info(
